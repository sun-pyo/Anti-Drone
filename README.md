...
>    Web 버전은 여러 사람이 동시에 접속할 수 있다는 장점이 있지만 비교적 동영상 속도가 느려지는 단점이 있다. 반대로 GUI 버전은 동영상 속도는 빠르지만 한 명의 사용자만 모니터링할 수 있다. 실시간 동영상 스트리밍 특성상 속도와 접근성 중 어느 것이 더 중요한지 결정하기 힘들어 두 버전 모두 구현하였다.
